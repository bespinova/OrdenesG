...
class modEdicion
{
  ...
  var $conDb;
  var $objParam;
  function __construct($curp,$NombreEm,$DireccionEm,$TelefonoEm,$CelularEm)
  {
    $this->curp = $curp;
    $this->NombreEm = $NombreEm;
    $this->DireccionEm = $DireccionEm;
    $this->TelefonoEm = $TelefonoEm;
    $this->CelularEm = $CelularEm;
  ...
  //  $this->objParam = new ClsParam($_SESSION['path']);
    ...
    public function EditarSave(){
        print_r($this->curp);
        $strUpd = 'UPDATE CatEmpleados set NombreEm = (?), DireccionEm = (?), TelefonoEm = (?), CelularEm = (?) WHERE Curp = (?)';
    // echo $strUpd;
        $this->ArrDat = array(
                    $this->NombreEm,
                    $this->DireccionEm,
