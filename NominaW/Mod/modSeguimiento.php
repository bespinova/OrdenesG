...
class modSeguimiento
{
  ...
  public function getLogin($usuario){
    $strSlc = "SELECT TOP 1 curp, CodEmpleado, Nombre, Autoriza,VacTotal,VacTomadas FROM CatEmpleados WHERE curp=  '".$usuario."'";
   ...
   // print_r($usuario);
    ...
    //print_r($rsp);
	 ...
	 public  function buscarSolVacEstatus($usuario){
	   // print_r($usuario);
		$stdSql = "select convert(varchar, FechaSol, 101) as FechaSol, tBen, firmaG, firmaRh, firmaGG 
					from catsolicitudes 
		...
		//print_r($rsp);
		 if (count($rsp) > 0){
			...
			return 'No tiene solicitudes pendientes....';
	 }
